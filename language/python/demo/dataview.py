...
def DataAllocation(datacmd, timecmd, ret):
    for i in range(int(ret)):
        i =i+1
        ret = subprocess.getoutput(timecmd + "| head -n {0} | tail -n 1".format(i))
        DataRet = subprocess.getoutput(datacmd + "|head -n {0} | tail -n 1".format(i))
        # ret = subprocess.getoutput("date -d '{0}' +%s".format(ret))      # unix time
        timelist.append(ret);datalist.append(float(DataRet))
    return 0

def cpuidledata(FILENAME):
    timecmd = "cat {0}| grep all | awk '{1}'".format(FILENAME, "{print$1,$2,$3}")
...
def unitcpudata(FILENAME, MODULE):
    if subprocess.getoutput("cat {0} | grep {1}".format(FILENAME, MODULE)) != "":
        ...
        exit(-1)
    time = "cat {0}| grep {1} | awk '{2}'".format(FILENAME, MODULE, "{print$1,$2,$3}")
    data = "cat {0}| grep {1} | awk '{2}'".format(FILENAME, MODULE, "{print$6}")
