...
# muisc = pygame.mixer.music.load("2022.mp3")
...
class part:
    def __init__(self, cv, idx, total, explosion_speed, x=0., y=0., vx=0., vy=0., size=100, color='red', lifespan=1,
                 **kwargs):
        self.id = idx
        self.x = x
        self.y = y
...
def simulate(cv):
    ...
    for point in range(numb_explode):
        ...
        size = uniform(3, 3)
        color = choice(colors)
        explosion_speed = uniform(5, 10)
        total_particles = randint(10, 30)
        for i in range(1, total_particles):
            r = part(cv, idx=i, total=total_particles, explosion_speed=explosion_speed, x=x_cordi, y=y_cordi,
                     vx=speed, vy=speed, color=color, size=size, lifespan=uniform(1, 1.6))
            objects.append(r)
...
if __name__ == '__main__':
    root = tk.Tk()
    cv = tk.Canvas(root, height=613, width=1000)
