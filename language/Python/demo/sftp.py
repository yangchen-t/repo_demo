...
logging.basicConfig(level=logging.INFO,#控制台打印的日志级别
                    filename='/home/qomolo/upload.log',
        ...
        while True:
            try:
                ...
                if len(current) < len(new_dir_list):
                    diff = set(new_dir_list).symmetric_difference(set(current))
                    for i in diff:
                        Process(target=check_diff_process, args=(i,)).start()
                    break
                else:
