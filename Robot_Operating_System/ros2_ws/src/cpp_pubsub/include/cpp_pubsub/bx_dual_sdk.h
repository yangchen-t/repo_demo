...
#pragma pack (push,1)
	...
	*/
	...
	/*下面的这个语音结构体BXSound_6G仅在动态区时使用；图文分区播放语音请使用：EQPicAreaSoundHeader_G6;*/
	typedef struct typeBXSound_6G
	{
	...
	BXSound_6G;/*这个语音结构体BXSound_6G仅在动态区时使用；图文分区播放语音请使用：EQPicAreaSoundHeader_G6;*/
	...
	typedef struct
	{
		Ouint8	AreaType;		//区域类型; 0x00	图文字幕区域; 0x06 普通背景; 0x08 渐变色背景; 动态区时，固定为0x10;
		...
		/*下面的这个语音结构体BXSound_6G仅在动态区时使用；图文分区播放语音请使用单独的结构体：EQPicAreaSoundHeader_G6;*/
		//语音内容
...
#pragma region Common operation 
	...
	******************************************************************/
	...
	BXDUAL_API Oint64 _CALL_STD UnicodeToUTF8(const Oint8* pStr, Oint8* pOutBuff, Oint64 nOutBufferSize);
	BXDUAL_API Oint64 _CALL_STD UnicodeToUTF8_V2(const Oint8* pInBuf, size_t* iInLen, Oint8* pOutBuf, size_t* iOutLen);
	...
	*/
	...
	通讯方式	：使用串口发送；
	参数	说明	：
	pSerialName		:串口号字符串；如:Oint8 pSerialName[] = "COM3";
	nBaudRateIndex	:波特率；取值为1时，代表波特率为9600; 取值为2时，代表波特率为57600;
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicArea_AddAreaTxtDetails_6G_Serial(Oint8* pSerialName, Oint8 nBaudRateIndex, E_ScreenColor_G56 color, Ouint8 uAreaId, EQareaHeader_G6* oAreaHeader_G6,
		EQpageHeader_G6* stPageHeader, Ouint8* fontName, Ouint8* strAreaTxtContent);
	...
	功能说明：动态区关联节目
	参数说明：
	Ouint8* pSerialName		: 串口名称,如"COM1"；
	Ouint32 nBaudRateIndex	: 波特率；取值为1时，代表波特率为9600; 取值为2时，代表波特率为57600;
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicArea_AddAreaTxtDetails_WithProgram_6G_Serial(Ouint8* pSerialName, Ouint32 nBaudRateIndex, E_ScreenColor_G56 color, Ouint8 uAreaId, EQareaHeader_G6* oAreaHeader_G6,
		EQpageHeader_G6* stPageHeader, Ouint8* fontName, Ouint8* strAreaTxtContent, Ouint16 RelateProNum, Ouint16* RelateProSerial);
	...
	Ouint32 nBaudRateIndex	: 波特率；取值为1时，代表波特率为9600; 取值为2时，代表波特率为57600;
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicArea_AddAreaPic_6G_Serial(Ouint8* pSerialName, Ouint32 nBaudRateIndex, E_ScreenColor_G56 color, Ouint8 uAreaId, Ouint16	AreaX, Ouint16 AreaY,
		Ouint16 AreaWidth, Ouint16 AreaHeight, EQpageHeader_G6* pheader, Ouint8* picPath);
	...
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicArea_AddAreaPic_WithProgram_G6_Serial(Ouint8* pSerialName, Ouint32 nBaudRateIndex, E_ScreenColor_G56 color, Ouint8 uAreaId, Ouint16 AreaX, Ouint16 AreaY,
		Ouint16 AreaWidth, Ouint16 AreaHeight, EQpageHeader_G6* pheader, Ouint8* picPath, Ouint16 RelateProNum, Ouint16* RelateProSerial);
	...
	Ouint32 nBaudRateIndex	: 波特率；取值为1时，代表波特率为9600; 取值为2时，代表波特率为57600;
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicAreaS_AddTxtDetails_6G_Serial(Ouint8* pSerialName, Ouint32 nBaudRateIndex, E_ScreenColor_G56 color, Ouint8 uAreaCount, DynamicAreaParams* pParams);
	...
	Ouint8* pSerialName		: 串口名称,如"COM1"；
	Ouint32 nBaudRateIndex	: 波特率；取值为1时，代表波特率为9600; 取值为2时，代表波特率为57600;
	Ouint16 RelateProNum	: = 0 时，关联所有节目，与所有节目一起播放，如果没有节目，则不播放该动态区；
							  > 0 时, 指定关联节目，要关联的节目ID存放在RelateProSerial[]中；
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicAreaS_AddTxtDetails_WithProgram_G6_Serial(Ouint8* pSerialName, Ouint32 nBaudRateIndex, E_ScreenColor_G56 color, Ouint8 uAreaCount, DynamicAreaParams* pParams, Ouint16 RelateProNum, Ouint16* RelateProSerial);
	...
	Ouint32 nBaudRateIndex	: 波特率；取值为1时，代表波特率为9600; 取值为2时，代表波特率为57600;
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicAreaS_AddAreaPic_6G_Serial(Ouint8* pSerialName, Ouint32 nBaudRateIndex, E_ScreenColor_G56 color, Ouint8 uAreaCount, DynamicAreaParams* pParams);
	...
	Ouint16 RelateProNum	: = 0 时，关联所有节目，与所有节目一起播放，如果没有节目，则不播放该动态区；
							  > 0 时, 指定关联节目，要关联的节目ID存放在RelateProSerial[]中；
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicAreaS_AddAreaPic_WithProgram_6G_Serial(Ouint8* pSerialName, Ouint32 nBaudRateIndex, E_ScreenColor_G56 color, Ouint8 uAreaCount, DynamicAreaParams* pParams, Ouint16 RelateProNum, Ouint16* RelateProSerial);
	...
	*/
	...
	nBaudRateIndex	:波特率；取值为1时，代表波特率为9600; 取值为2时，代表波特率为57600;
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicArea_AddAreaInfos_G6_Serial(Oint8* pSerialName, Oint8 nBaudRateIndex, E_ScreenColor_G56 color,
		Ouint8 uAreaId,
		Ouint8 RunMode,
		Ouint16 Timeout,
		Ouint8 RelateAllPro,
	...
	返回值：0 成功；-1 失败；
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicArea_AddAreaInfos_6G_V2(Ouint8* pIP, Ouint32 nPort, E_ScreenColor_G56 color,
		Ouint8 uAreaId,
		Ouint8 RunMode,
		Ouint16 Timeout,
		Ouint8 RelateAllPro,
	...
	nBaudRateIndex	:波特率；取值为1时，代表波特率为9600; 取值为2时，代表波特率为57600;
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicArea_AddAreaInfos_6G_V2_Serial(Oint8* pSerialName, Oint8 nBaudRateIndex, E_ScreenColor_G56 color,
		Ouint8 uAreaId,
		Ouint8 RunMode,
		Ouint16 Timeout,
		Ouint8 RelateAllPro,
	...
	*/
	...
	nBaudRateIndex	:波特率；取值为1时，代表波特率为9600; 取值为2时，代表波特率为57600;
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicArea_AddAreaWithTxt_5G_Serial(Ouint8* pSerialName, Ouint32 nBaudRateIndex, E_ScreenColor_G56 color,
		Ouint8 uAreaId,
		Ouint8 RunMode,
		Ouint16 Timeout,
		Ouint8 RelateAllPro,
	...
	nBaudRateIndex	:波特率；取值为1时，代表波特率为9600; 取值为2时，代表波特率为57600;
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicArea_AddAreaWithTxt_Point_5G_Serial(Ouint8* pSerialName, Ouint32 nBaundRateIndex, E_ScreenColor_G56 color,
		Ouint8 uAreaId,
		Ouint8 RunMode,
		Ouint16 Timeout,
		Ouint8 RelateAllPro,
	...
	*/
	...
	函数返回：
		...
		-1	：失败；
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicArea_AddAreaWithPic_5G_Serial(Ouint8* pSerialName, Ouint32 nBaudRateIndex, E_ScreenColor_G56 color,
		Ouint8 uAreaId,
		Ouint8 RunMode,
		Ouint16 Timeout,
		Ouint8 RelateAllPro,
	...
	*/
	...
	功能说明：增加多条信息（文本/图片）到指定的动态区，并可以关联这个动态区到指定的节目；
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicArea_AddAreaInfos_5G_Point(Ouint8* pIP, Ouint32 nPort, E_ScreenColor_G56 color,
		Ouint8 uAreaId,
		Ouint8 RunMode,
		Ouint16 Timeout,
		Ouint8 RelateAllPro,
	...
	函数返回：
		...
		-1	：失败；
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicArea_AddAreaInfos_5G_Serial(Ouint8* pSerialName, Ouint32 nBaudRateIndex, E_ScreenColor_G56 color,
		Ouint8 uAreaId,
		Ouint8 RunMode,
		Ouint16 Timeout,
		Ouint8 RelateAllPro,
	...
	*/
	...
	uAreaId = 0xff:删除所有区域
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicArea_DelArea_G5_Serial(Oint8* pSerialName, Oint8 nBaudRateIndex, Oint8 uAreaId);
	...
	uAreaCount-动态区ID数组中的个数；
	*/
	BXDUAL_API int _CALL_STD bxDual_dynamicArea_DelAreaS_G5_Serial(Oint8* pSerialName, Oint8 nBaudRateIndex, Oint8 uAreaCount, Oint8* pAreaID);
	...
	***特别提醒：给出的区域宽度一定大于要显示的内容，否则发送成功，但是会没有任何显示；（比如：有固定文字和单位时，要将这些文字的宽度考虑到区域宽度内）；
	...
	******************************************************************/
	...
	/*!
	 *  给区域设置单个背景;
	 *  背景类型分为：背景区和渐变色背景区;通过与这个函数配套使用的bxDual_program_addArea_G6接口函数中的AreaType指定；
	 *    
	 *  picPath			指定作为背景的图片的文件名完整路径;字符串类型;
	 *  picPathLen		指定图片的文件名完整路径长度;
	 *  DisplayMode		1）渐变色背景区数据格式时：0x00–随机显示;0x01–静止显示;0x02–快速打出;0x03–向左移动;0x04–向左连移;0x05–向上移动;0x06–向上连移;0x07–闪烁;...
	 *					2）背景区数据格式时：显示方式 0x01-–静止显示；0xfe--动画显示。
	 *	nSpeed			速度等级，0为最快速度
	 *  nStayTime		停留时间，单位为10ms;
	 *  
	 ...
	 *  背景类型分为：背景区和渐变色背景区;通过与这个函数配套使用的bxDual_program_addArea_G6接口函数中的AreaType指定；
	 */
	BXDUAL_API int _CALL_STD bxDual_progrm_AddBGAttribs_G6(Ouint16 areaID, Ouint8* picPath, Ouint32 picPathLen, Ouint8 DisplayMode, Oint8 nSpeed, Oint16 nStayTime);
