...
function time_search_copy_csv(){
    if [ ! -d "/home/qomolo/ws_cpc_ws" ];then
        echo "/home/qomolo/ws_cpc_ws路径不存在，请检查并创建"
        exit 0
    fi
    cd /home/qomolo/ws_cpc_ws
    search_time_stamp=`date -d "${SEARCH_TIME}" +%s`
    older_time_stamp=`expr ${search_time_stamp} - 7200`
    OLDER_SEARCH_TIME=`date -d @$older_time_stamp +%Y-%m-%d\ %H:%M:00`
    newer_time_stamp=`expr ${search_time_stamp} + 7200`
...
function Denmark_folder_search_co(){
    cd /home/qomolo/ws_cpc_ws/short_time
    ...
    find /home/qomolo/ws_cpc_ws/short_time -name 'CO*' -type d -newermt "$START_TIME" ! -newermt "$END_TIME"  | xargs -I {} cp -r {} /data/code/all_ws/ws/logpush_tmp/tmp/${TMP_LOG_NAME}/CO
}
...
function file_search_log(){
    cd /home/qomolo/ws_cpc_ws/qlog/$1
